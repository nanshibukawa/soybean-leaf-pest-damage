...
class ModelTrainingPipeline:
    ...
    def __init__(self, config: ModelConfig):
        ...
        Args:
            config (ModelConfig): Configuração do modelo contendo parâmetros de treinamento
        """
        self.config = config
    ...
    def main(self, stage3_result: dict, stage2_result: dict):
        ...
        Args:
            stage3_result (dict): Resultados da etapa 3 (salvamento do modelo)
            stage2_result (dict): Resultados da etapa 2 (divisão dos dados)

        Returns:
            dict: Resultados do treinamento
            - success (bool): Indica se o treinamento foi bem-sucedido
            - model (tf.keras.Model): Modelo treinado
            - history (tf.keras.callbacks.History): Histórico do treinamento
            - metrics (dict): Métricas de desempenho do treinamento
...
if __name__ == "__main__":
    try:
        ...
        logger.info(
            f">>>>>> stage {MODEL_STAGE_NAME} requires stage 2 and 3 results <<<<<<"
        )
    except Exception as e:
        logger.exception(e)
        raise e
