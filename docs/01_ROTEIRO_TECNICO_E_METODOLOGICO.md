...
5. **Diagnóstico de Ruído Amostral e Polimorfismo Biológico (*Spodoptera albula*):** Auditoria empírica nos recortes minerados via API revelou espécimes na fase adulta (mariposas) e artefatos de detecção ao lado de lagartas canônicas. Essa distribuição bimodal explica o gap de generalização no teste externo zero-shot do INSECT12C (estritamente composto por lagartas) e serve de fundamentação para a discussão de erros do manuscrito. Detalhes em [`04_experimentos_e_benchmark/04_analise_qualitativa_ruido_mineracao_e_polimorfismo.md`](04_experimentos_e_benchmark/04_analise_qualitativa_ruido_mineracao_e_polimorfismo.md).
